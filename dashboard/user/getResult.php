...
class PDF extends FPDF {
        ...
        global $admno;
         ...
         while ($row17 = mysqli_fetch_array($result17)) {
             ...
             while ($row18 = mysqli_fetch_array($result18)) {
                 $avg_current = $avg_current + $row18['firstTest'] + $row18['secondTest'] + $row18['assignment'] + $row18['project'] + $row18['exam'];
                 $counter++;
             }
    ...
    function header(){
        ...
        $this->cell(70,20,$this->Image('sign/cof.png',180,10,24),0,0,'C');
        ...
        $this->Ln(4);
        $this->cell(190,5,'AWAKA OWERRI NORTH L.G.A IMO STATE',0,0,'C');
        $this->Ln(4);
    ...
    function viewTable($conn){
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            $this->cell(15,6,$row['firstTest'],1,0,'C');
            $this->cell(15,6,$row['secondTest'],1,0,'C');
            $this->cell(20,6,$row['assignment'],1,0,'C');
            $this->cell(17,6,$row['project'],1,0,'C');
            $this->cell(15,6,$row['exam'],1,0,'C');
            ...
            $this->Ln();
            $this->SetFont('Times','',8);
            $this->cell(64,6,'Access Bank',1,0,'C');
